...
settings["identifier"] = ut.create_identifier("motion correction example 2")
...
dg = f.make_group("devices")
dg.set_dataset("<device_X>", "Information about device goes here", name="Alice's Acme 2-photon microscope")
op = f.make_group("optophysiology")
c1 = op.make_group("<imaging_plane_X>", "camera1")
c1.set_dataset("device", "Alice's Acme 2-photon microscope")
...
dg.set_dataset("<device_X>", "Information about device goes here", name="Bob's Acme 2-photon microscope")
c2 = op.make_group("<imaging_plane_X>", "camera2")
c2.set_dataset("device", "Bob's Acme 2-photon microscope")
c2.set_dataset("excitation_lambda", "1000 nm")
c2.set_dataset("indicator", "GCaMP6s")
...
orig = f.make_group("<TwoPhotonSeries>", "Alice's source image", path="/acquisition/timeseries")
orig.set_attr("description", "Pointer to a 640x480 image stored in the file system")
orig.set_attr("source", "Data acquired from Alice's Acme 2-photon microscope")
orig.set_dataset("format", "external")
orig.set_dataset("external_file", ["/path/to/file/alice_stack.tif"], attrs={
    "starting_frame": [0]})
...
mc.set_custom_dataset("random comment", "Note that the 'original' field under 2photon is an HDF5 link to the time series '/acquisition/timeseries/Alice's source image'")
...
orig = f.make_group("<TwoPhotonSeries>", "Bob's source image", path="/acquisition/timeseries")
orig.set_attr("description", "Pointer to a 640x480 image stored in the file system")
orig.set_attr("source", "Data acquired from Bob's Acme 2-photon microscope")
orig.set_dataset("format", "external")
orig.set_dataset("external_file", ["/path/to/file/bob_stack.tif"], attrs={
    "starting_frame": [0]})
orig.set_dataset("bits_per_pixel", 16)
orig.set_dataset("dimension", [640, 480])
...
mc.set_custom_dataset("a comment", "Note that the 'original' field under 2photon is an HDF5 link to the time series '/acquisition/timeseries/Bob's source image'")
